...
class OrcLazyJIT {
public:
  #define ORC_LAZY_CACHE_FOLDER_NAME  ".@@@orc-jit-cache@@@/"  //Added by cyf 2018-10-18(cache folder name)
  ...
  OrcLazyJIT(std::unique_ptr<TargetMachine> TM,
             ...
             bool InlineStubs, ObjectCache *ObjCache) // Added by cyf 2018-10-15(for cache)
      : TM(std::move(TM)), DL(this->TM->createDataLayout()),
        CCMgr(std::move(CCMgr)),
        ObjectLayer([]() { return std::make_shared<SectionMemoryManager>(); }),
        CompileLayer(ObjectLayer, orc::SimpleCompiler(*this->TM,ObjCache)),	// Added by cyf 2018-10-15(for cache)
...
int runOrcLazyJIT(std::vector<std::unique_ptr<Module>> Ms,
                  ...
                  char **pArgV = nullptr,
                  unsigned *pUnit_arg_len = nullptr); //Modified by cyf 2018-10-19(nullptr mean run main function)
